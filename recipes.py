...
Pete can bake (integer). For simplicity there are no units for the amounts (e.g. 1 lb of flour or 200 g of sugar are simply 1 or 200). 
...
def cakes(recipe, available):
    ...
    for name in recipe.keys():
        if name not in available.keys():
          return 0
        else:
          res = available[name] // recipe[name]
          if res == 0:
            return 0
          lst.append(res)
    num = min(lst)
...
print(cakes({'flour': 500, 'eggs': 5, 'bacon': 150},{'flour':1000, 'eggs': 10 }))

#assert(cakes({'flour': 500, 'eggs': 5, 'bacon': 150},
#{'flour':1000, 'eggs': 10 }) == 0))
